...
> 不能用 name 属性的值作为判断，有可能标识符与该值相同

1. 函数的 return 语句存在判断   
   如果返回语句在 selectionStmt 中，怎么判断

2. statementList 其实可以不用兄弟节点保存，child 列表大小足够保存

3. checkNode 中类型检查
   - 对于判断式左右的（返回）类型怎么判断为int  
   直接拿到类型就好了，但是左右的值可能还是个表达式
...
| AST | 选择该选项后，点击“生成”按钮后，语法分析输出抽象语法树 |
| NST | 选择该选项后，点击“生成”按钮后，语法分析输出普通语法树 |
